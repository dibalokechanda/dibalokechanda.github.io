...
$$
P(X \mid \text { rest of the nodes })=P(X \mid MB (X))
$$

where, $MB(X)$ is the Markov blanket consisting of the node's parents, children, and children's parents. Mathematically, it can be written as:
